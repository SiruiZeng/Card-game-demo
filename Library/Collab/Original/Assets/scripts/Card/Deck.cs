...
public class Deck {
    ...
    public void loadCard(TextAsset cardFile)
    {
        line = cardFile.text;
    ...
    public void deleteAdv(Card aCard)
    {
        Card theCard = advanture.Find(x => x.getName() == aCard.getName());
        if (theCard != null)
        {
            advanture.Remove(theCard);
        }
        else
        {
            Debug.Log("failed to delete adv card");
        }
    }
}
