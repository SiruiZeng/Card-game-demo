...
public class gameControl : MonoBehaviour
{
	...
	private int joinCount = 0; //how many player been asked if join.
    ...
    public void getCard(float distance, string aName, int which)
    {
        ...
        if (which == 1)
        {
            Vector3 toPos = pos_p1 + new Vector3(distance, 0, 1f) * (p1.getHand().Count - 1);//p1 wrong
            iTween.MoveTo(toMove, toPos, 2);
        }
        if (which == 0)
        { 
            iTween.MoveTo(toMove, pos_story, 0.8f);
        }
	...
	public void askIfJoin(int stat)
    {
		if (joinCount < 2) {
			 ...
			 *2. view to first player in quest
			 *3. display playcards msg and btns.
			 **/
    ...
    public void switchView()
    {
        for (int i = 0; i < CURRENTPLAYERNUM; i++)
        {
            if (i == (currentView + 1))
            {
                for (int j = 0; j < players[i].getHand().Count; j++)
                {
                    string cardName = players[i].getHand()[j].getName();
                    getCard(5, cardName, 1);
